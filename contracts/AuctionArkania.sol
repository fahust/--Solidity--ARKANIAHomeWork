...
contract AuctionContract is Ownable {
    ...
    mapping(string => uint256) paramsContract; //Allows great scalability, you can add or remove as many variables as you want
    ...
    function setRoyaltiesAdress(address _address) internal {
        _addressRoyalties[countAddressRoyalties] = RoyaltiesAddresses(
            _address,
            true
        );
    ...
    function removeRoyaltiesAdress(address _address) external {
        for (
            uint8 iRoyalties = 0;
            iRoyalties <= countAddressRoyalties;
            iRoyalties++
        ) {
    ...
    */
    ...
    function mintDelegate() external payable {
    ...
    function getTokenDetails(uint256 tokenId)
        external
        view
        returns (TokenArkania.Token memory)
    {
        TokenArkania contrat = TokenArkania(addressContractToken);
    ...
    function bidding() external payable {
        require(
            msg.value > paramsContract["bid"],
            "Require more ETH for bidding"
        );
        require(paramsContract["startAuction"] != 0, "Auction not started");
        require(
            block.timestamp < (paramsContract["startAuction"] + 10 minutes),
            "Auction is finished"
        );
    ...
    function getAllBidders() external view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](countBidder);
    ...
    function getBidderDetails(uint256 bidderId)
        external
        view
        ...
        for (uint256 i = 0; i <= countBidder; i++) {
            if (lastBidder == _bidders[i].addressBidder) {
                for (uint256 iToken = 0; iToken < 10; iToken++) {
                    contrat.transfer(
                        contrat.getOwnerOf(iToken),
                        _bidders[i].addressBidder,
                        iToken
                    );
                }
            } else {
    ...
    function payRoyalties() internal {
        ...
        for (
            ...
            if (_addressRoyalties[iRoyalties].valid == true) {
                payable(_addressRoyalties[iRoyalties].addressRoyalties)
                    .transfer(percent);
            }
        }
    }
